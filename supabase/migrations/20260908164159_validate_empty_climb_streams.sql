create or replace function public.analyze_cycling_stream(samples jsonb) returns jsonb
...
begin
  ...
  n:=cardinality(tt);
  if n is null or n<2 or n>200000 then return jsonb_build_object('version','climbs_v1','status','invalid','segments',result); end if;
  ...
  if cardinality(dd) is distinct from n or cardinality(aa) is distinct from n then return jsonb_build_object('version','climbs_v1','status','invalid','segments',result); end if;
