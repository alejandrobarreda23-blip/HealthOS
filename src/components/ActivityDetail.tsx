...
import ActivityHeartRate from './ActivityHeartRate';
export function ActivityDetail({ session: s, sessions, userId, onOpenBody, colorMode = 'intensity' }: { colorMode?: ActivityColorMode; session: TrainingSession; sessions: TrainingSession[]; userId: string; onOpenBody: (date: string) => void }) {
  ...
  return <article className="card activityDetail">
    ...
    <div className="activityTabs" aria-label="Detalle del entrenamiento">{[['summary','Sesión'],['heart','Frecuencia cardíaca'],['compare','Comparar'],['context','Contexto']].map(([key,label]) => <button key={key} aria-pressed={tab === key} onClick={() => setTab(key)}>{label}</button>)}</div>
    {tab === 'heart' && <ActivityHeartRate session={displaySession} sessions={sessions} source={source} userId={userId}/>}
    {tab === 'summary' && <>
      ...
      <section className="activityRouteNotice"><strong>{displaySession.summary.hasRouteReference ? 'Ruta identificada en origen' : 'Recorrido GPS no importado'}</strong><p>{displaySession.summary.hasRouteReference ? 'El registro contiene una referencia de ruta, pero no sus coordenadas. Todavía no podemos dibujar el recorrido.' : 'Esta sesión no tiene un trazado GPS guardado en HealthOS.'}</p></section><h3>Qué permite analizar esta sesión</h3><p>Volumen registrado, desnivel y resumen cardiaco. Abre Frecuencia cardíaca para explorar zonas, solicitar la curva y comparar el esfuerzo con otras sesiones. La media y el máximo, por sí solos, no reconstruyen la evolución temporal.</p>
