  ...
  return <>
    ...
    {report && !history.loading && !context.loading && <>
      <WeeklyLearningPanel report={report} memories={context.memory?.memories ?? []} following={context.memory?.following ?? []} onOpenTrend={onOpenTrend ? openTrend : undefined}
        checkInForm={scope?.isSelf ? <details className="weeklyCheckIn"><summary>¿Cómo te encontrabas en esos días? Registrar energía y fatiga</summary><label className="weeklyContextDate">Día <input type="date" min={report.start} max={end} value={checkDate >= report.start && checkDate <= end ? checkDate : end} onChange={e => setCheckDate(e.target.value)}/></label><CheckIn key={`${userId}|${checkDate}`} date={checkDate >= report.start && checkDate <= end ? checkDate : end} onSaved={context.refresh}/></details> : <p className="weeklyReadOnlyContext">Estás consultando otro perfil. Aquí aparecerán las sensaciones que registre su titular.</p>}
      ...
      {scope?.isSelf ? <><p className="weeklySaveStatus">La última semana completa se guarda al abrir esta vista. Puedes guardar también una semana histórica recalculada; la fecha de guardado queda visible.</p><button className="secondary" disabled={!canSave} onClick={save}>Guardar esta revisión</button></> : <p className="weeklySaveStatus">Vista de lectura. Puedes consultar las revisiones guardadas; el contexto y el seguimiento los modifica la persona titular del perfil.</p>}
    </>}
