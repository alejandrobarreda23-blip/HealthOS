...
export default function EvolutionExplorer({metric,points,baselines,events,history,end,days,onOpenActivities}: {
  ...
  const dailyPaths=useMemo(()=>{const groups:TrendPointV1[][]=[];for(const row of [...points].sort((a,b)=>a.date.localeCompare(b.date))){const previous=groups.at(-1)?.at(-1);if(!previous||dayOffset(row.date,previous.date)!==1||previous.sourceKey!==row.sourceKey)groups.push([]);groups.at(-1)!.push(row);}return groups;},[points]);
