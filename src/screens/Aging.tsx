...
import VisibleAging from '../components/aging/VisibleAging';
...
export default function Aging({onOpenTrend}:{onOpenTrend?:(key:string)=>void}){
  ...
  return <div className="agingScreenV2">
    <header className="pageHeader agingHeaderV2">
      <div>
        <div className="eyebrow">AGING</div>
        <h1>{mode==='visible'?'Visible Aging':'Trayectoria'}</h1>
        <p className="muted pageLead">{mode==='visible'
          ?'Cómo cambia la expresión externa del organismo con mínima carga para el usuario, sin mezclar apariencia con edad biológica.'
          :'Cómo cambia el organismo a través de sistemas distintos, separando observación, cobertura y madurez del modelo.'}</p>
      </div>
      <div className="agingHeaderActions">
        <div className="agingModeSwitch" aria-label="Modo Aging">
          <button className={mode==='physiology'?'active':''} onClick={()=>setMode('physiology')}><Activity size={15}/>Fisiológico</button>
          <button className={mode==='visible'?'active':''} onClick={()=>setMode('visible')}><Eye size={15}/>Visible</button>
        </div>
        <div className="agingMaturityPill"><ShieldQuestion size={16}/>Runtime experimental</div>
      </div>
    </header>
