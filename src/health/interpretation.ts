...
export interface Narrative { title: string; observation: string; connection: string; meaning: string; next: string; personal?: string; level: 'description' | 'first' | 'repeated' | 'inconsistent' | 'insufficient'; sources: { title: string; url: string }[]; }
...
export function questionNarrative(q: WeeklyQuestion): Narrative {
  ...
  const stableAnswer = q.id === 'sleep-pulse' ? 'Dormir más no se acompaña de un cambio consistente en tu pulso' : q.id === 'steps-next-sleep' ? 'Caminar más no se acompaña de un cambio consistente en la duración del sueño' : 'Dos noches más cortas no se acompañan de un cambio consistente en tu HRV';
  const measuredAnswer = last && last.delta !== 0 ? q.id === 'steps-next-sleep' ? `Después de caminar más, registraste ${differenceText(q.y, last.delta!)} de sueño` : q.id === 'sleep-pulse' ? `En las noches más largas, tu pulso fue ${differenceText(q.y, last.delta!).replace(/ menos$/, 'más bajo').replace(/ más$/, 'más alto')}` : `Tras dos noches más cortas, tu HRV fue ${differenceText(q.y, -last.delta!)}` : 'En los días comparados no apareció una diferencia';
  const status = q.state === 'inconsistent' || q.state === 'no_difference' ? `Por ahora, ${stableAnswer[0].toLowerCase()}${stableAnswer.slice(1)}` : q.state === 'first_signal' ? `${measuredAnswer} · primera comparación` : q.state === 'repeated' ? `${measuredAnswer} · se repite en dos periodos` : 'Todavía faltan días comparables';
  const dates = last ? `${last.matches.length} pares de días en la última comparación · ${shortDate(last.start)} – ${shortDate(last.end)}.` : '';
  const reference = q.reference ? ` «Más» se refiere a superar tu referencia de ${formatMonitoring(q.x, q.reference.threshold)}.` : '';
  return { title: status, observation: last ? `${dates} ${q.state === 'inconsistent' || q.state === 'no_difference' ? `En la última comparación: ${differenceText(q.y, last.delta!)} ${q.y === 'resting_heart_rate' ? 'de pulso en las noches más largas' : q.y === 'sleep_duration' ? 'de sueño después de los días con más pasos' : 'de HRV tras dos noches más largas'}. ` : 'La cifra resume la diferencia dentro de cada pareja. '}${reference}` : observation, level: q.state === 'repeated' ? 'repeated' : q.state === 'first_signal' ? 'first' : q.state === 'inconsistent' ? 'inconsistent' : q.state === 'no_difference' ? 'description' : 'insufficient',
    personal: q.state === 'repeated' ? 'Merece seguirse: la diferencia reaparece con días nuevos. Todavía falta comprobar si coincide con más energía o menos fatiga en tu caso.' : q.state === 'first_signal' ? 'Aún no sabemos si esta diferencia es habitual en ti. La próxima comparación usará días nuevos.' : undefined,
